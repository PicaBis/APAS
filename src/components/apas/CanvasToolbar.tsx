  ...
  return (
    <div className={isFullscreen
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 text-xs text-muted-foreground flex-wrap">
        <span className="text-xs sm:text-sm font-medium">{T.playbackSpeed}</span>
        {[0.10, 0.25, 0.5, 1, 2, 4].map((s) => (
          <button key={s} onClick={() => { onSetPlaybackSpeed(s); playSpeedChange(isMuted); }}
            className={`apas-btn px-2 sm:px-2.5 py-1 rounded-lg text-xs sm:text-sm font-mono border transition-all duration-300 ${playbackSpeed === s ? 'bg-primary text-primary-foreground border-primary shadow-md shadow-primary/20' : 'hover:bg-primary/10 border-transparent hover:border-primary/20'}`}>
            {s}x
          </button>
        ))}
      </div>
